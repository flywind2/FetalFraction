...
while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    sample=${location##*/}
    echo $sample
    zcat --force $location/*.fastq* | bowtie2 --very-sensitive -q -x $REF -p 10 | samtools view -q 35 -S - | cut -f3,4 | Rscript seqff.sam.R | (printf "$sample\t" && cat) >> results.test1.sam.tsv
done < "$locations"
